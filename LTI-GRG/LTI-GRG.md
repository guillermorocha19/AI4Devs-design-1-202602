...
| PA-002.3 | Definir el mecanismo exacto de integración con LinkedIn para la importación de perfil (OAuth, API de Partners, etc.). | Ingeniería + Partnerships | Pendiente |
...
| PA-003.4 | Definir el nivel de detalle del feedback constructivo al candidato descartado (automático vs. escrito por el reclutador). | Product Manager + UX | Pendiente |
...
erDiagram
    ...
    Reclutador ||--o{ OfertaEmpleo : "gestiona"
    ...
    CV ||--o| PerfilEstructurado : "genera tras parsing"
...
- **Candidato ↔ PerfilEstructurado**: Relación 1:1 porque el documento establece que un candidato tiene un único perfil estructurado que se enriquece con cada CV o importación de LinkedIn (US-002.1, US-002.2). El perfil acumula datos, no se duplica por candidatura.
- **CV ↔ PerfilEstructurado**: Relación 1:0..1 porque un CV genera un perfil parseado, pero una importación de LinkedIn (campo `origen` en PerfilEstructurado) crea el perfil directamente sin CV. El campo `origen` distingue si el perfil proviene de un CV parseado o de una importación de LinkedIn.
- **Candidatura como entidad pivote**: Vincula Candidato con Oferta y contiene el scoring agregado, el estado en el pipeline y la referencia a la etapa actual. Es la entidad central del flujo de screening (PRD-003).
...
- **Notificacion**: Cubre la comunicación automatizada (sección 2.1, punto 6) y las notificaciones con feedback constructivo al candidato (US-003.5, customer journey del candidato fases 4-5-8).

**Cambios respecto a la versión anterior y justificación:**

- **AuditLog reemplaza a AuditTrail**: La nueva entidad responde a las 5W (quién, qué, cuándo, dónde, cómo): `actor_id` + `actor_tipo` (quién — soporta tanto reclutadores como acciones del sistema), `accion` + `entidad` + `entidad_id` (qué), `fecha` (cuándo), `ip_origen` + `user_agent` (dónde), `datos_antes` + `datos_despues` en JSON (cómo). Registra modificaciones y consultas sobre cualquier entidad del modelo, no solo sobre candidaturas. Se elimina la relación directa con Candidatura para que AuditLog sea una entidad transversal independiente que audita todo el sistema.
...
| **L3 Component** | Módulo de Aplicación (Portal) | 4 sub-componentes (L4) | US-002.1, US-002.2, US-003.2, CJ candidato fase 3 |
| **L4 Code** | CV Uploader | — | US-002.1 |
| | LinkedIn Importer | — | US-002.2 |
| | Killer Questions Form | — | US-003.2 |
| | Application Orchestrator | — | CJ candidato fase 3 (<3 clics) |

### 7.4 Notas sobre Decisiones del Modelo

- **Nivel 4 (Code) aplicado al Módulo de Aplicación**: Se eligió este componente para el detalle L4 porque es el punto de mayor abandono según la documentación (92% de candidatos no completan su solicitud, sección 1.2) y el customer journey del candidato (fase 3) define requisitos específicos: aplicación en menos de 3 clics, dos vías de entrada (CV y LinkedIn) y killer questions integradas. Es el componente donde la descomposición a nivel de código aporta más valor.
